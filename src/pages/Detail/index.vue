          ...
          <div class="goodsDetail">
            ...
            <div class="support">
              ...
              <div class="supportArea">
                ...
                <div class="fixWidth">广东省 深圳市 南山区</div>
              </div>
            </div>
          </div>
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                <dt class="title">{{ spuSaleAttr.saleAttrNamdte }}</dt>
                <dd
                  changepirce="0"
...
export default {
  ...
  mounted() { //内存中的模板已挂载到页面中
    this.$store.dispatch('getProductDetails', this.$route.params.commodityId)
  },
  computed: {
    ...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrList']),
    //给子组件的数据
    skuImageList() {
      //如果服务器数据没有回来,skuImageList这个对象是空
      return this.skuInfo.skuImageList || {}
    }
  },
  methods: {
    changeActive(saleAttrValue, arr) {
      //遍历全部售卖属性的isChecked为零
      arr.forEach(element => {
        element.isChecked = 0;
      });
      //点击的哪个售卖属性值isChecked为一
    ...
    async addShopcar() {
      try {
        ...
        //产品信息的数据[比较复杂的数据,通过会话存储(不持久化,会话结束数据消失)]
