...
export default {
  ...
  computed: {
    ...mapState({
      ...
      orderNumber: (state) => state.trade.orderNumber
    }),
    //提交订单最终地址
    userDefaultAddress() {
      //find:查找数组中符合条件的元素返回
    ...
    //   let sum = 0;
    //   console.log(this);
    //   let total = this.orderInfo.detailArrayList
    //   total.forEach((item) => (sum += item.orderPrice));
    //   return sum;
    // }
  },
  methods: {
    ...
    changeDefault(address, addressInfo) {
      addressInfo.forEach((item) => (item.isDefault = 0))//全部的isDefault为零
      address.isDefault = 1;
    },
    //提交订单
    async submitOrder() {
      ...
      try {
        ...
        this.$router.push('/pay?orderId=' + this.orderNumber)
      } catch (error) {
        //提交订单失败
