<template>
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a @click="deleteAllCheckdCart">删除选中的商品</a>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      switch (type) {
        case 'add':
          ...
          break;
        case 'minus':
          //判定产品的个数大于1,才可以传递给服务器-1,反之为 0
          disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        case 'change':
          //用户输入进来的最终量,如果非法的(带有汉字或负数),带给服务器数字为零
          if (isNaN(disNum) || disNum < 1) {
            disNum = 0;
          } else {
            //小数取整,带给服务器的值为(最终量-起始个数)
    ...
    async updateCheck(cart, event) {
      ...
      try {
        ...
        await this.$store.dispatch('updateCheckdById', { skuId: cart.skuId, isChecked: checked })
        //如果修改状态成功,再次获取购物车数据
        this.getData()
      } catch (error) {
        alert(error.message);
  ...
  computed: {
    ...
    //判定底部复选框是否勾选
    isAllCheck() {
      //遍历数组里面的isChecked属性(只要有一个为1返回的就为假)
